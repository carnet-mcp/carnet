...
# One ticket, built the way `core/approvals.py` builds one, with a fixed request id so the
# decision half of each pair above can name it without threading a return value through a
# lambda.
_APPROVAL_REQUEST = "REQ-000000000114"


def _approval(fingerprint: str = "fp-114"):
    from carnet.storage.base import make_approval_record

    return make_approval_record(
        _APPROVAL_REQUEST, fingerprint, AGENT["name"], "post_message",
        {"channel": "#eng"}, "machine", "tok-1",
    )
...
IN_SCOPE = (
    ...
    # Step 114. The two entries whose subject is a ticket: a person deciding that one
    # call an agent's permission list holds back may (or may not) happen. Both go through
    # `claim_approval` first, because a ticket only exists once something asked for it.
    (
        "approval.grant",
        lambda s, t: (
            s.claim_approval(
                t, _approval(), call_id="door-1",
                grant_window=timedelta(minutes=15), request_window=timedelta(hours=24),
            ),
            s.decide_approval(
                t, _APPROVAL_REQUEST, decision="grant", by_kind="user", by_id="u-1",
                note="", grant_window=timedelta(minutes=15), actor="user:u-1",
            ),
        ),
    ),
    (
        ...
        lambda s, t: (
            ...
            s.decide_approval(
                t, _APPROVAL_REQUEST, decision="deny", by_kind="user", by_id="u-1",
                note="not today", grant_window=timedelta(minutes=15), actor="user:u-1",
            ),
        ),
    ),
    (
...
def _populate(store, tenant_id, rid_for):
    ...
    # Migration 057. A pending approval is product state and cascades; it deliberately
    # does **not** join `TENANT_BLOCKING_TABLES`, because a ticket nobody answered must
    # not be able to hold up a customer's deletion. What proves the cascade is the
    # `information_schema` walk, and the walk proves nothing about a table with no row in
    # it — which is the whole reason this line is here rather than in one test.
    _ask(store, tenant_id, fingerprint=f"fp-{tenant_id}")
    return tenant_id
...
def test_a_suspended_tenant_is_deleted_with_everything_in_it(store, tenant, rid):
    ...
    assert store.approval_requests(tenant) == []
...
# --- approvals, migration 057 --------------------------------------------------------
#
# The one table in this schema with a lifecycle, so these assertions are about **state
# transitions** rather than about a record's shape. The windows are parameters precisely
# so that expiry can be driven rather than waited for: a negative grant window is an
# approval that was already stale when it was given, and a zero request window is a
# denial whose binding has already elapsed. Nothing here sleeps.

_GRANT = timedelta(minutes=15)
_REQUEST = timedelta(hours=24)


def _ask(store, tenant, *, fingerprint="fp-1", call_id="door-aaaa", request_id=None,
         principal_id="tok-1", tool="post_message", grant=_GRANT, request=_REQUEST):
    from carnet.storage.base import make_approval_record, new_request_id
    ...
    return store.claim_approval(
        tenant, record, call_id=call_id, grant_window=grant, request_window=request
    )


def test_a_first_ask_opens_a_pending_ticket(store, tenant):
    result = _ask(store, tenant)

    assert result["outcome"] == "pending"
    assert result["request_id"].startswith("REQ-")

    (row,) = store.approval_requests(tenant)
    assert row["state"] == "pending"
    assert row["asked_count"] == 1
    assert row["agent"] == AGENT["name"]
    assert row["arguments"] == {"channel": "#eng"}
    assert row["granted_until"] is None
    assert row["spent_call_id"] == ""


def test_asking_again_refreshes_the_same_ticket_rather_than_opening_a_second(store, tenant):
    """One row per distinct call, forever — migration 057's header.

    The alternative is a row per attempt, which is a table a granted token fills at will
    and a screen listing one request thirty times. `asked_count` carries the fact the
    duplicates would have carried, in the shape the approver's question is asked in."""
    first = _ask(store, tenant)
    second = _ask(store, tenant)
    third = _ask(store, tenant)

    assert second["request_id"] == first["request_id"] == third["request_id"]
    assert [r["asked_count"] for r in store.approval_requests(tenant)] == [3]


def test_a_different_caller_is_a_different_ticket(store, tenant):
    """The caller is in the fingerprint: a yes for the staging token must not admit
    production's. The fingerprints differ because `core/approvals.py` puts the principal
    in them; here they are supplied, which is what makes this an assertion about the
    *table* rather than about the hash."""
    a = _ask(store, tenant, fingerprint="fp-staging")
    b = _ask(store, tenant, fingerprint="fp-prod")

    assert a["request_id"] != b["request_id"]
    assert len(store.approval_requests(tenant)) == 2


def test_a_yes_admits_exactly_one_call_and_is_then_spent(store, tenant):
    """The whole control. The second identical call finds the yes gone and asks again."""
    opened = _ask(store, tenant)
    store.decide_approval(
        tenant, opened["request_id"], decision="grant", by_kind="user", by_id="u-1",
        note="ok", grant_window=_GRANT, actor="user:u-1",
    )

    spent = _ask(store, tenant, call_id="door-bbbb")
    assert spent["outcome"] == "granted"
    assert spent["request_id"] == opened["request_id"]

    (row,) = store.approval_requests(tenant)
    assert row["state"] == "spent"
    assert row["spent_call_id"] == "door-bbbb"

    again = _ask(store, tenant, call_id="door-cccc")
    assert again["outcome"] == "pending"
    (row,) = store.approval_requests(tenant)
    assert row["state"] == "pending"
    # The reopen clears what the last decision left, so nothing reads as approved by a
    # person who approved something else.
    assert row["granted_until"] is None
    assert row["decided_by_id"] == ""
    assert row["spent_call_id"] == ""


def test_a_grant_whose_window_has_passed_is_not_spendable(store, tenant):
    """Expiry is the clock compared against a column, never a state somebody swept — so
    a window that was already over when the yes was given behaves exactly as one that
    elapsed while nobody was looking."""
    opened = _ask(store, tenant)
    store.decide_approval(
        tenant, opened["request_id"], decision="grant", by_kind="user", by_id="u-1",
        note="", grant_window=timedelta(seconds=-1), actor="user:u-1",
    )

    assert _ask(store, tenant)["outcome"] == "pending"


def test_a_refusal_binds_until_its_window_passes(store, tenant):
    """*No, not today* and *no, never* are different answers, and this table can only
    honestly store the first."""
    opened = _ask(store, tenant)
    store.decide_approval(
        tenant, opened["request_id"], decision="deny", by_kind="user", by_id="u-1",
        note="not this one", grant_window=_GRANT, actor="user:u-1",
    )

    refused = _ask(store, tenant)
    assert refused["outcome"] == "denied"
    assert refused["decided_by_id"] == "u-1"
    assert refused["decided_at"] is not None

    # The same call once the binding has elapsed: the question may be asked again, under
    # the id it was always asked under.
    reopened = _ask(store, tenant, request=timedelta(0))
    assert reopened["outcome"] == "pending"
    assert reopened["request_id"] == opened["request_id"]
    (row,) = store.approval_requests(tenant)
    assert row["note"] == ""
    assert row["decided_at"] is None


def test_deciding_a_ticket_twice_is_refused(store, tenant):
    """A screen shows a yes button for a request somebody else answered thirty seconds
    ago, and that has to be a sentence rather than a second approval."""
    opened = _ask(store, tenant)
    store.decide_approval(
        tenant, opened["request_id"], decision="grant", by_kind="user", by_id="u-1",
        note="", grant_window=_GRANT, actor="user:u-1",
    )

    with pytest.raises(ValueRefused, match="already granted"):
        store.decide_approval(
            tenant, opened["request_id"], decision="deny", by_kind="user", by_id="u-2",
            note="", grant_window=_GRANT, actor="user:u-2",
        )


def test_deciding_a_request_that_does_not_exist_answers_none(store, tenant):
    """None rather than a refusal: the route turns it into a 404, and an id that never
    existed and one that aged out are the same fact to whoever is holding the link."""
    assert store.decide_approval(
        tenant, "REQ-000000000000", decision="grant", by_kind="user", by_id="u-1",
        note="", grant_window=_GRANT, actor="user:u-1",
    ) is None


def test_a_decision_is_recorded_without_the_arguments(store, tenant):
    """The ticket is mutable; `admin_audit` is where the decision stops being erasable.

    And the detail names the agent, the tool and the window — never the arguments, which
    are the same class of content `AGENT_DETAIL_REDACTED` keeps out of a record kept
    forever, and which `audit` already holds under the tool's own redaction."""
    opened = _ask(store, tenant)
    store.decide_approval(
        tenant, opened["request_id"], decision="grant", by_kind="user", by_id="u-1",
        note="", grant_window=_GRANT, actor="user:u-1",
    ...
    (record,) = store.admin_audit_records(tenant, action="approval.grant")
    assert record["target_kind"] == "approval"
    assert record["target_id"] == opened["request_id"]
    assert record["detail"]["agent"] == AGENT["name"]
    assert record["detail"]["tool"] == "post_message"
    assert record["detail"]["minutes"] == 15
    assert "#eng" not in json.dumps(record["detail"])


def test_an_empty_agent_filter_returns_nothing_and_none_returns_everything(store, tenant):
    """The difference between *no agents* and *no filter* is this method's whole safety:
    the screen passes the agents the caller holds `editor` on, and somebody holding none
    must see none rather than all."""
    _ask(store, tenant, fingerprint="fp-a")
    _ask(store, tenant, fingerprint="fp-b")

    assert store.approval_requests(tenant, agents=[]) == []
    assert len(store.approval_requests(tenant, agents=None)) == 2
    assert len(store.approval_requests(tenant, agents=[AGENT["name"]])) == 2
    assert store.approval_requests(tenant, agents=["someone-elses-agent"]) == []


def test_approvals_are_filtered_by_state_and_ordered_by_the_last_ask(store, tenant):
    first = _ask(store, tenant, fingerprint="fp-a")
    _ask(store, tenant, fingerprint="fp-b")
    # `fp-a` asked again, so it is the most recently asked despite being the older row.
    _ask(store, tenant, fingerprint="fp-a")

    assert [r["fingerprint"] for r in store.approval_requests(tenant)] == ["fp-a", "fp-b"]

    store.decide_approval(
        tenant, first["request_id"], decision="grant", by_kind="user", by_id="u-1",
        note="", grant_window=_GRANT, actor="user:u-1",
    )
    pending = store.approval_requests(tenant, states=["pending"])
    assert [r["fingerprint"] for r in pending] == ["fp-b"]
    assert len(store.approval_requests(tenant, states=["granted"])) == 1


def test_an_approval_is_scoped_to_its_tenant(store, tenant, other):
    _ask(store, tenant, fingerprint="shared-fingerprint")
    _ask(store, other, fingerprint="shared-fingerprint")

    assert len(store.approval_requests(tenant)) == 1
    assert len(store.approval_requests(other)) == 1


def test_an_approval_for_an_unknown_tenant_is_refused_by_both_stores(store):
    """The foreign key in Postgres, `_require_tenant` in the fake — and the fake had to
    be told, which is the shape of drift this file exists for."""
    from carnet.storage.base import make_approval_record, new_request_id

    record = make_approval_record(
        new_request_id(), "fp", "a", "t", {}, "machine", "tok-1"
    )
    with pytest.raises((UnknownTenantError, StorageError)):
        store.claim_approval(
            "no-such-tenant", record, call_id="door-1",
            grant_window=_GRANT, request_window=_REQUEST,
        )


def test_a_record_a_column_would_refuse_is_refused_by_the_builder(store, tenant):
    """`check_denial_resource_kind`'s device: a vocabulary reachable only through the
    builder is one the *public* method accepts in the fake and the column refuses in
    Postgres."""
    from carnet.storage.base import make_approval_record, new_request_id

    with pytest.raises(StorageError, match="request id"):
        make_approval_record("4b2f", "fp", "a", "t", {}, "machine", "tok-1")
    with pytest.raises(StorageError, match="fingerprint"):
        make_approval_record(new_request_id(), "", "a", "t", {}, "machine", "tok-1")
    with pytest.raises(StorageError):
        make_approval_record(new_request_id(), "fp", "a", "t", {}, "group", "g-1")

    with pytest.raises(StorageError):
        store.decide_approval(
            tenant, "REQ-x", decision="maybe", by_kind="user", by_id="u-1",
            note="", grant_window=_GRANT, actor="user:u-1",
        )
