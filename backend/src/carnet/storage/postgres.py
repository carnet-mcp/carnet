...
class PostgresStorage:
    ...
    _APPROVAL_COLUMNS = ("id",) + APPROVAL_FIELDS[1:]  # id, then all but tenant_id

    # The spend, and **the control**. One conditional UPDATE rather than a read the caller
    # acts on: it takes the row lock, and exactly one of two identical calls arriving
    # together comes back with a row. A `SELECT … then UPDATE` admits both at every
    # isolation level this product runs at, and the failure that produces — one approval,
    # two payments sent — is the failure the feature exists to prevent.
    #
    # The window is in the predicate rather than checked afterwards, so a grant nobody
    # used in time is simply not there to spend. That is the same decision
    # `approval_display_state` makes for a reader: expiry is the clock compared against a
    # column, never a state somebody swept.
    # **The clock is the database's**, here and in every statement below, and that is
    # not incidental: `datetime` is imported in this module under `TYPE_CHECKING` only,
    # because a store that mints its own timestamps is a store whose answers move when an
    # app server's clock does. Two replicas disagreeing by a minute would mean an approval
    # spendable on one and expired on the other. `now()` is transaction time, so the spend
    # and the ask below compare against one instant.
    _APPROVAL_SPEND = """
        UPDATE approvals
           SET state = 'spent', spent_at = now(), spent_call_id = %(call_id)s
         WHERE tenant_id = %(tenant)s AND fingerprint = %(fingerprint)s
           AND state = 'granted' AND granted_until > now()
     RETURNING request_id
    """

    # The ask, opened or refreshed. The CASE **is** the state machine, and it is in SQL
    # for the reason the UPDATE above is: the alternative reads the row, decides in
    # Python, and writes — which is the race one statement up, wearing a different hat.
    #
    # A granted-but-expired row and a spent row reopen on the next ask. A denial reopens
    # only once its window has passed, which is what stops a refused agent re-queueing the
    # same question at the same person all afternoon. The request id is **kept** through a
    # reopen: it is what somebody has already searched for, and a second id for one call
    # would make one request's trail read as two.
    _APPROVAL_ASK = """
        INSERT INTO approvals (
            tenant_id, v, request_id, fingerprint, agent, tool, arguments,
            principal_kind, principal_id, acting_for, state,
            requested_at, last_asked_at, asked_count,
            decided_by_kind, decided_by_id, decided_at, note,
            granted_until, spent_at, spent_call_id
        ) VALUES (
            %(tenant)s, %(v)s, %(request_id)s, %(fingerprint)s, %(agent)s, %(tool)s,
            %(arguments)s, %(principal_kind)s, %(principal_id)s, %(acting_for)s,
            'pending', now(), now(), 1, '', '', NULL, '', NULL, NULL, ''
        )
        ON CONFLICT (tenant_id, fingerprint) DO UPDATE SET
            asked_count   = approvals.asked_count + 1,
            last_asked_at = now(),
            state = CASE
                WHEN approvals.state IN ('granted', 'spent') THEN 'pending'
                WHEN approvals.state = 'denied'
                     AND approvals.decided_at <= now() - %(request_window)s
                THEN 'pending'
                ELSE approvals.state END,
            decided_by_kind = CASE
                WHEN approvals.state = 'pending'
                     OR (approvals.state = 'denied'
                         AND approvals.decided_at > now() - %(request_window)s)
                THEN approvals.decided_by_kind ELSE '' END,
            decided_by_id = CASE
                WHEN approvals.state = 'pending'
                     OR (approvals.state = 'denied'
                         AND approvals.decided_at > now() - %(request_window)s)
                THEN approvals.decided_by_id ELSE '' END,
            decided_at = CASE
                WHEN approvals.state = 'pending'
                     OR (approvals.state = 'denied'
                         AND approvals.decided_at > now() - %(request_window)s)
                THEN approvals.decided_at ELSE NULL END,
            note = CASE
                WHEN approvals.state = 'pending'
                     OR (approvals.state = 'denied'
                         AND approvals.decided_at > now() - %(request_window)s)
                THEN approvals.note ELSE '' END,
            granted_until = NULL,
            spent_at      = NULL,
            spent_call_id = ''
     RETURNING request_id, state, decided_at, decided_by_id
    """
    ...
    def claim_approval(
        ...
        *,
        ...
        # Both statements on one connection inside one transaction. The spend has already
        # committed its row lock by the time the ask runs, so the two cannot see a state
        # between them — the property the fake gets from holding its lock across both.
        with self._transaction() as cur:
            spent = cur.execute(
                self._APPROVAL_SPEND,
                {
                    "call_id": call_id,
                    "tenant": tenant_id,
            ...
            if spent is not None:
                return {"outcome": "granted", "request_id": spent[0]}
            ...
            row = cur.execute(
                self._APPROVAL_ASK,
                {
                    ...
                    # `requested_at`, `last_asked_at`, `asked_count` and the four decision
                    # columns are **not** taken from the record: the INSERT above spells
                    # them, because a row is born pending at the database's now and
                    # nothing may hand this method a ticket that is already decided.
                    "request_window": request_window,
                },
            ).fetchone()

        request_id, state, decided_at, decided_by_id = row
        if state == "denied":
            return {
                "outcome": "denied",
    ...
    def approval_requests(
        ...
        *,
        ...
        columns = ", ".join(self._APPROVAL_COLUMNS)
        where = ["tenant_id = %(tenant)s"]
        params: dict = {"tenant": tenant_id}

        # `= ANY(%s)` with an empty list is a predicate that matches nothing, which is
        # exactly what an empty `agents` must mean — see the protocol. Written as a
        # parameter rather than as an interpolated IN list for the obvious reason and for
        # one more: an IN list built from zero elements is a syntax error, so the shape
        # that would have to special-case the dangerous value is the one that does not.
        if agents is not None:
            where.append("agent = ANY(%(agents)s)")
            params["agents"] = list(agents)
        if states is not None:
            where.append("state = ANY(%(states)s)")
            params["states"] = list(states)

        sql = (
            f"SELECT {columns} FROM approvals"  # noqa: S608 - module constants only
            f" WHERE {' AND '.join(where)}"
            " ORDER BY last_asked_at DESC, id DESC"
        )
        ...
        return [
            {**dict(zip(self._APPROVAL_COLUMNS, row)), "tenant_id": tenant_id}
    ...
    def decide_approval(
        ...
        *,
        ...
        with self._transaction() as cur:
            # `FOR UPDATE`, because the screen this serves has a yes button and two people
            # looking at it. Without the lock, two grants of one ticket both read
            # `pending` and both write — and the second silently replaces the first
            # approver's name in the row while `admin_audit` records them both, which is a
            # log that disagrees with the state it describes.
            current = cur.execute(
                f"SELECT {columns} FROM approvals"  # noqa: S608 - module constants only
            ...
            row = dict(zip(self._APPROVAL_COLUMNS, current))
            if row["state"] != "pending":
                raise ValueRefused(
                    f"{request_id} is already {row['state']} and cannot be decided "
            ...
            updated = cur.execute(
                f"UPDATE approvals SET state = %s, decided_by_kind = %s, "  # noqa: S608
                "decided_by_id = %s, decided_at = now(), note = %s, "
                "granted_until = CASE WHEN %s THEN now() + %s ELSE NULL END "
                f"WHERE tenant_id = %s AND request_id = %s RETURNING {columns}",
                (
                    "granted" if decision == "grant" else "denied",
                    by_kind,
                    by_id,
                    note,
                    decision == "grant",
            ...
            # In the same transaction as the write it describes — `_write_admin`'s whole
            # argument, and its other direction too: a record the database refuses takes
            # this decision down with it rather than leaving a ticket granted by nobody.
            self._write_admin(cur, tenant_id, record)

        return {**dict(zip(self._APPROVAL_COLUMNS, updated)), "tenant_id": tenant_id}
